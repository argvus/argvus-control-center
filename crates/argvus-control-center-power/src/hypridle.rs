...
fn lock_command(content: &str) -> String {
  content
    ...
    .unwrap_or_else(|| "sh /usr/share/argvus/scripts/apps/hypr-power-menu.sh --lock".to_string())
}
...
";
    ...
    assert_eq!(listener_minutes_from(&updated, calls_dpms_off), Some(5));
    assert!(updated.contains("lock_cmd = sh /usr/share/argvus/scripts/apps/hypr-power-menu.sh --lock"), "{updated}");
    assert!(!updated.contains("on-timeout = sh /usr/share/argvus/scripts/apps/hypr-power-menu.sh --lock"), "{updated}");
  }

  #[test]
  fn calls_lock_matches_lock_idioms_but_not_sleep_or_dpms() {
    assert!(calls_lock("sh /usr/share/argvus/scripts/apps/hypr-power-menu.sh --lock"));
