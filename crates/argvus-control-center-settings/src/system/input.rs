...
impl InputSettings {
  fn load_runtime_values(&mut self) {
    ...
    self.touchpad.sensitivity =
      query_float("input:touchpad:sensitivity").unwrap_or(self.touchpad.sensitivity);
...
impl InputSettings {
  pub fn toggle(&mut self, index: usize) -> Result<(), String> {
    let (key, value) = match index {
      ...
      9 => ("input:touchpad:tap_to_click", !self.touchpad.tap_to_click),
      10 => ("input:touchpad:tap_and_drag", !self.touchpad.tap_and_drag),
      11 => (
    ...
    match index {
      ...
      8 => self.touchpad.natural_scroll = value,
      9 => self.touchpad.tap_to_click = value,
      10 => self.touchpad.tap_and_drag = value,
      11 => self.touchpad.two_finger_right_click = value,
      12 => self.touchpad.disable_while_typing = value,
      _ => {}
    }
    save(self)
  }
    ...
    match index {
      1 | 7 => {
        let target = if index == 1 {
          &mut self.mouse.sensitivity
        } else {
          &mut self.touchpad.sensitivity
        };
        *target = validate_sensitivity(*target + f64::from(direction) * 0.1)?;
        let key = if index == 1 {
          "input:sensitivity"
        } else {
          "input:touchpad:sensitivity"
        };
        apply(key, &target.to_string())?;
      }
      2 => {
