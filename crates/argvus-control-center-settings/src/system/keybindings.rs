...
pub fn save_override(
  ...
  let mut file = canonical_overrides()
    .or_else(|| {
      fs::read_to_string(config_path())
        .ok()
        .and_then(|v| toml::from_str::<Overrides>(&v).ok())
  ...
  write_canonical_overrides(&file)?;
  generate(bindings)
}
...
pub fn restore_all(bindings: &[Binding]) -> Result<(), String> {
  ...
  write_canonical_overrides(&Overrides {
    version: 1,
    keybindings: BTreeMap::new(),
  })?;
  generate(bindings)
...
fn canonical_overrides() -> Option<Overrides> {
  let output = Command::new("argvus-config")
    .args(["get", "/hyprland/keybindings", "--raw"])
    .output()
    .ok()?;
  if !output.status.success() {
    return None;
  }
  let keybindings = serde_json::from_slice(&output.stdout).ok()?;
  Some(Overrides {
    version: 1,
    keybindings,
  })
...
fn write_canonical_overrides(overrides: &Overrides) -> Result<(), String> {
  let value = serde_json::to_string(&overrides.keybindings).map_err(|error| error.to_string())?;
  match Command::new("argvus-config")
    .args(["set", "/hyprland/keybindings", &value])
